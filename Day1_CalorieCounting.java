...
public class Day1_CalorieCounting {
     ...
     * uses a flag variable to get around limite var scope of catch
     *
     * @return
    ...
    public static int topElf() throws FileNotFoundException {
        ...
        while (sc.hasNextLine()) {
            ...
            try {
                ...
                // it's an int - update counter and set valid to 1
                counter += next;
                valid = 1;
            } catch (Exception e) {
            } finally {
                if (valid == 0) {  // it was a blank line
                    // see if the elf's calories are the max
                    if (counter > max) {
                        max = counter;
                    }
                    //reset counter
        ...
        //close scanner
        sc.close();
        // return the max number of calories
        return max;
